...
if (!hook("replaceslideshow")) 
		...
		function nextPhoto()
		    ...
		    if (!document.getElementById('image1')) {return false;} /* Photo slideshow no longer available (AJAX page move) */
			  ...
			  if (flip==0)
			    ...
			    window.setTimeout("image1.style.backgroundImage='url(<?php echo $baseurl . "/" . $homeanim_folder?>/" + next_photo + ".jpg?checksum=<?php echo $checksum ?>)';if(linktarget!=''){jQuery('#slideshowlink').attr('href',linktarget);}else{jQuery('#slideshowlink').removeAttr('href');}",1000);
		     	flip=1;
		     	}
			  else
			  	{
	...
	if ($small_slideshow && !$slideshow_big) 
		...
		<div id="SlideshowContainer">
			...
			<div id='image1' alt='' style="background:url(<?php echo $baseurl . '/' .  $homeanim_folder?>/2.jpg?checksum=<?php echo $checksum ?>); display:none;
                        <?php
			if (isset($home_slideshow_width)){
