...
// else check svnversion	
$svnrevision=shell_exec("svnversion ". $storagedir."/..");	
$svnrevision=explode(":",$svnrevision);

if (isset($svnrevision[1])){$svnrevision=$svnrevision[1];}
else {$svnrevision=$svnrevision[0];}
...
if ($files_modified){
	...
	?><br/><br/><table class="InfoTable"><?php
	for ($n=1;$n<count($svn_status);$n++){
		if ($svn_status[$n]["text_status"]==8){
		?>
		<tr><td><?php echo $svn_status[$n]["path"]?></td><td><b><?php echo "modified"?></b></td></tr><?php	
		}
	}
	?>
	</table><?php
...
$svnlog=svn_log  ("http://svn.resourcespace.org/svn/resourcespace",  
SVN_REVISION_HEAD, 
$svnrevision+0);
...
if (count($svnlog)>1){
	...
	for ($n=0;$n<count($svnlog);$n++){
		if (isset($svnlog[$n])){?>
		<tr><td><a href="svnup.php?rev=<?php echo $svnlog[$n]["rev"]?>">&gt;&nbsp;<?php echo $lang["updatetothisrevision"]?></a></td><td><?php echo $svnlog[$n]["rev"]?></td><td><?php echo $svnlog[$n]["author"]?></td><td><b><?php foreach($svnlog[$n]["paths"]as $svnpath){echo $svnpath["path"];}?></b></td><td nowrap><b><?php echo date("d-m-Y",strtotime($svnlog[$n]["date"]))?></b></td><td><b><?php echo $svnlog[$n]["msg"]?></b></td></tr><?php	
		}
	}	
?></table><?php
}
