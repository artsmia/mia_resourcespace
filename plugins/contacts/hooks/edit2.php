...
function HookContactsEdit2Autolivejs(){
global $contact_fields;
?>
<style type="text/css">
  .Question{position:relative;}
  #box{background: rgba(255,255,255,0.8);border: 1px solid;padding: 0; width: 200px;position:absolute;z-index:10000;top:0;right:0; max-height: 300px;overflow: auto;}
  #box li{padding:0;}
...
jQuery(document).ready(function(){
  ...
  for(f=0; f<fieldstofocus.length; f++){
    jQuery(fieldstofocus[f]).addClass('contacts').prop("autocomplete","off").prop("placeholder","Enter a contact...");
  }
  ...
  jQuery('.contacts').keyup(function(){
    ...
    jQuery("#box").hover(function(){
      triggered = true;
      jQuery(document).bind("keypress.key9", function(e){var code = e.keyCode || e.which;if (code  == 9) {e.preventDefault();return false;}});
    },
    function(){triggered = false;jQuery(document).unbind('keypress.key9');}
    );
  });//end keyup()
